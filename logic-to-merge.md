...
1. **Clear Project Structure**:
   - Separate directories for source code (`democracy_exe/`), tests (`tests/`), documentation (`docs/`).
...
8. **Test Organization**:
   - Tests should be in the `tests/` directory, mirroring the structure of the `democracy_exe/` directory.
...
```toml
# … other configs[tool.ruff.lint]
select = [
    "D200", # fits-on-one-line: One-line docstring should fit on one line (stable)
    "E226", # missing-whitespace-around-arithmetic-operator: Missing whitespace around arithmetic operator (unstable)]     # other rule numbers …. etc]# other configs …```
