...
def __(Path, mo, os, prompt_library_module):
    ...
    COMPARE_DIR = "one-off-tasks/lore-writing/helldivers2/johnhelldiver/compare"
    PROMPT_DIRS = [
        "./src/prompt_library/data/prompt_lib",
        "./one-off-tasks"
    ]
    ...
    try:
        with mo.status.spinner(title="Loading XML prompts from all directories..."):
            map_prompt_library: dict = prompt_library_module.pull_in_multiple_prompt_libraries(
                directories=[Path(d) for d in PROMPT_DIRS],
                file_type="xml"
            )
...
def __(QUESTIONS_DIR, glob, mo, os):
    ...
    # Create dropdown for question selection and text input
    question_selector = mo.ui.dropdown(
        options=question_names,
        value="None",
        label="Select a predefined question (or None to write your own)"
    )
...
@app.cell
def __(question_selector, question_input, read_question_file):
    """Update question input when selector changes."""
...
@app.cell
def __(map_prompt_library, mo, llm_module):
    ...
    mo.stop(not map_prompt_library, mo.md("""
...
def __(all_placeholders, mo, placeholder_array, proceed_button):
    ...
    filled_values = dict(zip(all_placeholders, placeholder_array.value))
